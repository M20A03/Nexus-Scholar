...
module.exports = (req, res) => {
  ...
  const { id, search, query, q, page = 1, limit = 50 } = req.query;
  ...
  const searchQuery = (search || query || q || '').toLowerCase().trim();
  ...
  if (searchQuery) {
    filtered = filtered.filter(p =>
      ...
      (p.authors && p.authors.some(a => a.name.toLowerCase().includes(searchQuery)))
    );
  }
